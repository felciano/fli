...
class DateSearchFilters(BaseModel):
    ...
    def validate_to_date(cls, v: str) -> str:
        ...
        Anchored to :func:`earliest_searchable_date` rather than the server's
        naive today: a range ending "today" is still live for any traveler west
        of the machine running the search.
        """
        to_date = datetime.strptime(v, "%Y-%m-%d").date()
    ...
    def validate_and_adjust_from_date(self) -> "DateSearchFilters":
        ...
        # The clamp target is plain "today", not the rejection floor: anchoring
        # it to earliest_searchable_date() would widen MAX_PAST_FROM_DATE_DAYS
        # by a day. It is UTC-anchored so the window does not shift with TZ.
